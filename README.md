...
Both metrics will be used as much as possible.

### [Assignment of cost function penalties for classification errors](#assignment-of-cost-function-penalties-for-classification-errors)
As a simple example of how penalties are assigned, consider an average principal of $1,000 and an average loan income of $100 (if a loan is paid off on time)
1. FN: a = default, p = on time, penalty is -1,000 (loss)
   - investor does fund loan
   - there was no return to be made
   - model incorrectly predicted this, so funding (principal) was provided
   - principal was lost
   - return was never there to be made, so return was not lost
   - -$1000
2. FP: a = on-time, p = default, penalty is -$100 (loss)
   - investor does not fund loan
   - there was return to be made
   - model incorrectly predicted this, so funding (principal) was not provided
   - return was missed
   - return was there to be made, so return was lost
   - -$100
3. TN: a = on-time, p = on-time, penalty is $100 (gain)
   - investor does fund loan
   - there was return to be made
   - model correctly predicts this, so funding (principal) was provided
   - return was made
   - return was there to be made, and this did happen
   - +$100
4. TP: a = default, p = default, penalty is 0
   - investor does not fund loan
   - there was no return to be made
   - model correctly predicted this, funding (principal) was not provided
   - nothing was lost
   - return was never there to be made, so return was not lost
   - $0

Adding the product of each type of error and its corresponding penalty gives the total cost function (or return earned/lost by an investor) due to making funding decisions based on the output of the ML model developed here.
